...
> **Phase 270 (2026-09-24) — contract changed below.** The email-OTP provider
> was retired and removed from the product. Consequently
> `PRODUCTION_EMAIL_TRANSPORT` is retired as a release prerequisite (see §D of
> the live contract): the manifest has **four** mandatory prerequisites.
> Phase-by-phase sections below the contract that narrate a five-prerequisite
> gate are *point-in-time history* and are intentionally left unedited.
...
**RETIRED (Phase 270).** This section described the prerequisites for
real email-OTP delivery (provider account, verified sender domain, DNS, a
mailbox). The email-OTP provider was removed from the product, so none of
those external prerequisites can ever matter again; the configuration names
it listed (`XSTARZ_EMAIL_*`) are inert and read by nothing. The entry is
preserved (struck) for traceability; `docs/PRODUCTION-EMAIL-SETUP.md` is
marked retired as well. `XSTARZ_DEPLOYMENT_ENV` and `SITE_URL` remain live
requirements — they are not email concerns.

*Retired text:* a Resend or SMTP2GO account with a verified domain, SPF/DKIM/
DMARC records, and a human-openable mailbox would have been required;
`XSTARZ_EMAIL_TRANSPORT`, `XSTARZ_EMAIL_API_KEY`, `XSTARZ_EMAIL_SENDER_ADDRESS`,
`XSTARZ_EMAIL_SENDER_NAME` (optional), and `XSTARZ_EMAIL_TIMEOUT_MS` would have
been the configuration names.
...
**RETIRED (Phase 270).** D1 required an actually-received email, a human
attestation of receipt, and an OTP session created through the application.
The email-OTP provider is gone, so D1 is permanently unobservable — stronger
than BLOCKED: impossible. This note replaces the former BLOCKED status.
...
## Phase 215 — DEV email configuration gate — **RETIRED (Phase 270)**

> Phase 270 addendum: the gate is gone, not unresolved. `XSTARZ_EMAIL_*` names
> no longer exist in any inventory, so the record below (unknown presence,
> Case A/B decision) is moot and is preserved only for traceability.
...
## Phase 217 — Evidence-D closed at the external provisioning boundary — superseded (Phase 270)

> Phase 270 addendum: D1 is terminally NOT_VERIFIED (recorded), not merely
> BLOCKED. The "external provisioning" framing is superseded — there is no
> longer a mechanism that provisioning could feed. The D2–D10 harness path
> (anonymous sign-in) is unaffected by this note.
...
**CLOSED (provider timestamp).** `convex/coinglass.fetchDerivatives` previously stamped `timestamp: Date.now()` on every successful payload, so Phase 226's "provider observation timestamp preserved" contract was false: delayed CoinGlass data graded FRESH at the radar (`assessFreshness` of the request clock). The payload `timestamp` is now the oldest surviving-leg CoinGlass `time`/`t`/`timestamp`/`createTime` (ms or UNIX seconds, same windows as Twelve Data). A missing/implausible provider time is `0` (UNAVAILABLE to the radar), never re-dated. Cache hits still replay the original observation. Covered in `coinglass-legs.phase228.test.ts`. No new product phase.
...
3. **CLOSED.** `liveProtection.ts` `detectAssetClass` `[\/\-]` → `[/-]`; the two `no-useless-escape` findings are gone. See Phase 230 §M-4.
...
No new `Date.now()` as provider observation. FX success `observedAt` derives from the real quote time (`direct?.timestamp ?? inverse?.timestamp`). **CLOSED (E2 sibling).** `fetchPair` previously stamped `timestamp: Date.now()` on a successful Twelve Data `/quote`, so the "real quote time" was the request clock and `?? Date.now()` could never observe a provider instant. It now uses the quote's own `timestamp` (UNIX seconds, same window as `resolveProviderPriceTimestamp` / Phase 227 `fetchPrice`); a missing or implausible time is `malformed`, uncached, never re-dated. Covered in `fx-rate-legs.phase230.test.ts` and `provenance-fabrication.phase220.test.ts`. This was out of Phase 230's original failure-class scope and is closed here without opening a new product phase. Treasury/EIA/OKX-spec action-level `observedAt`/`acquisition` untouched. The marketData primary path (Phase 220 provider-observed price time) untouched. The live-quote `.catch(() => null)` (item M.2) is unchanged.
...
1. **CLOSED.** `classifyFailure` envelope-path timeout-text: word-boundary `timeout` / spaced `timed out` / `deadline exceeded|elapsed` now classify as `timeout` (not `unavailable`). Envelope-path retries stop on timeout the same way the thrown path already did. Socket `ETIMEDOUT` remains `network`. Rate-limit text still outranks timeout text. Covered in `provider-resilience.phase177.test.ts`. This was out of Phase 230's original scope (shared Phase 177 module) and is closed here without opening a new product phase.
...
3. **CLOSED.** `protectedAnalysis.ts` slow-group wrapper (`budgeted`) now maps a failed optional-slow leg through `optionalSlowEnvelope`, which forwards `{success:false, error: "<category>: <redacted reason>"}` instead of dropping the class. `fetchOptionalSlowData` still treats any non-success as `undefined` (never a fallback). Covered in `provider-resilience.phase177.test.ts` and a source pin in `fanout-integration.phase177.test.ts`. This was out of Phase 230's original scope and is closed here without opening a new product phase.
4. **CLOSED.** `liveProtection.ts` `detectAssetClass` character class `[\/\-]` → `[/-]` (still `/` or `-` after a listed ticker). The two `no-useless-escape` findings are gone; regex meaning unchanged. tests/scripts `no-explicit-any` remain out of scope per phase brief.
5. **CLOSED.** `fetchFxRate` `fetchPair` no longer stamps `timestamp: Date.now()` (Phase 220 E2 class, same `/quote` endpoint Phase 227 fixed in `twelve-data.fetchPrice`). Provider UNIX-seconds `timestamp` is scaled to ms; missing/implausible time is malformed and is not cached. See §H.
...
### D. The manifest — four mandatory prerequisites
...
*(Phase 270: the former fifth prerequisite, `PRODUCTION_EMAIL_TRANSPORT`, was
retired. It proved a real email delivery path for email-OTP sign-in; Phase
270 removed that provider, so there is no capability left for the proof to
establish. The id stays retired — it is not reusable, and an evidence record
filing it counts as an **unrecognised claim** under rule 3.)*
...
*(`PRODUCTION_EMAIL_TRANSPORT` was retired as a prerequisite in Phase 270
and no longer appears in this list; see §D.)*
...
| `npm run release:report` | **0** | the full report, ending in `NOT ADMITTED: 4 mandatory prerequisite(s) are not VERIFIED` (Phase 270: the email-transport prerequisite was retired) |
...
| Attempt | Result |
|---|---|
| an empty evidence object | refused; all four mandatory prerequisites are blockers |
