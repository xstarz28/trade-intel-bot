...
> **Phase 270 note — OTP limiter now FROZEN.** The email-OTP sign-in path that
> this limiter protected was retired: no provider can request an OTP email
> anymore, so `otpLimiter.consumeResendAllowance` has no live caller by design.
> The limiter, the `otpResendBuckets` table and `abuseLimiterPolicy.ts` are
> **retained frozen** for data safety (existing bucket rows keep meaning until
> they expire through the retention job); they must not be deleted or re-wired
> outside an explicit data-lifecycle decision. Everything below describes the
> mechanism as built in Phase 187 and stays accurate for readers of the
> retained code.
