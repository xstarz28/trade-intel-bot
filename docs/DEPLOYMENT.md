...
| --- | --- | --- |
| ~~`XSTARZ_EMAIL_API_KEY`~~ | **(RETIRED — Phase 270)** | There is no email sign-in path anymore: email-OTP was retired, the variable is inert and read by nothing |
| ~~`XSTARZ_EMAIL_SENDER_ADDRESS`~~ | **(RETIRED — Phase 270)** | Same — no sending capability exists to refuse or fall back |
...
| --- | --- | --- |
| `XSTARZ_EMAIL_API_KEY` | **NEVER** | *(Retired in Phase 270 but unchanged rule)* — a credential, if one was ever issued. Printing it discloses it, including into shell history and transcripts. Inspect **presence only**. |
| `XSTARZ_EMAIL_SENDER_ADDRESS` | Yes | *(Retired in Phase 270.)* A sending identity was published in message headers. It was operationally sensitive, not secret. |
| `XSTARZ_EMAIL_TRANSPORT` | Yes | *(Retired in Phase 270.)* Was one of `resend`, `smtp2go`, `console`. Any value is inert now. |
| `XSTARZ_EMAIL_SENDER_NAME` | Yes | *(Retired in Phase 270.)* Display name. |
...
| ~~`XSTARZ_EMAIL_TRANSPORT`~~ | **Removed in Phase 270** — there is no transport inventory anymore; any value is inert |
| ~~`XSTARZ_EMAIL_API_KEY`~~ | **Removed in Phase 270** — nothing reads it; a still-set value reports as unexpected |
| ~~`XSTARZ_EMAIL_SENDER_ADDRESS`~~ | **Removed in Phase 270** — no sending capability exists |
| ~~`XSTARZ_EMAIL_SENDER_NAME`~~ | **Removed in Phase 270** |
...
~~Email stays in Convex production env (never GitHub)~~ — **RETIRED (Phase 270).**
No `XSTARZ_EMAIL_*` variable belongs anywhere anymore: the email-OTP provider
and its delivery stack were removed, so set nothing and delete nothing —
still-set values are inert and report as unexpected in the config inventory.
...
| 3 | ~~`PRODUCTION_EMAIL_TRANSPORT`~~ | **RETIRED (Phase 270)** — the prerequisite was removed from the release gate entirely: there is no email transport left to verify. Evidence still claiming this id surfaces as an *unrecognised claim*, never as a gate row. Status: **RETIRED** | — no action possible or needed |
...
Until items 1, 2 and 4 are cleared, this product is **NOT READY** for public release.
...
# Phase 270: the XSTARZ_EMAIL_* settings retired with the email-OTP provider —
# the guards they fed are gone, and the variables themselves are inert.
...
| ~~`XSTARZ_EMAIL_TRANSPORT=console` on production~~ | **RETIRED (Phase 270)** — no email boundary exists to evaluate the value |
| ~~`XSTARZ_EMAIL_TRANSPORT=resend` with no key~~ | **RETIRED (Phase 270)** — no transport exists to deliver or fail |
| ~~sender on `resend.dev` (or a subdomain) on production~~ | **RETIRED (Phase 270)** — no sender identity exists at all |
...
— **RETIRED (Phase 270):** the module and its string-addressed limiter call no
longer exist anywhere in the tree, so the "wait for codegen" rewrite it looked
forward to is moot. The limiter tables stay, retained-frozen.
