...
| ~~`EVIDENCE_D_EMAIL`~~ | ~~A real mailbox you can open, for the OTP~~ | **RETIRED (Phase 270)** — the OTP mechanism no longer exists |
| ~~`XSTARZ_EMAIL_TRANSPORT`~~ | ~~Read only to detect the non-delivering `console` transport~~ | **RETIRED (Phase 270)** — read by nothing anymore |
...
| `--auth anonymous` | The only session mechanism. Development only; mints a fresh identity per run. `--auth otp` is **refused**: the email-OTP provider was retired (Phase 270) and cannot establish a session. |
...
| # | Observation |
| --- | --- |
| D1 | **RETIRED (Phase 270)** OTP email received at a real mailbox — permanently unobservable (no provider) |
...
D1 — a human attesting that an OTP email reached a real mailbox — can never be
observed again: the email-OTP provider was removed, so no code can be sent and
no mailbox can receive one. The harness records D1 as `NOT_VERIFIED` with the
retirement as the reason. It is preserved in the D1–D10 list only so the
retired fact stays visible; a run with D1 impossible is, by definition, a
development-observation run.

*History:* D1 was human-attested; a console transport pretended to deliver and
was forbidden. The fail-closed instinct survives in the harness refusing any
`--production-evidence` run outright: with OTP gone, this harness has no
scriptable production session mechanism, so it cannot manufacture production
evidence at all.
