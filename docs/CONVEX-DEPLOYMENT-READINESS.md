...
| `XSTARZ_EMAIL_TRANSPORT` | **unset** | preflight FAIL |
| `XSTARZ_EMAIL_API_KEY` | **unset** | preflight FAIL |
| `XSTARZ_EMAIL_SENDER_ADDRESS` | **unset** | preflight FAIL |
...
The runtime does not depend on the generated entry resolving: `emailOtp.ts`
reaches the limiter through a string-addressed function reference
(`"otpLimiter:consumeResendAllowance"`) precisely because the generated `internal.*`
path was unavailable when the limiter was written. That workaround should be
replaced with `internal.otpLimiter.consumeResendAllowance` after official
codegen runs.
...
| --- | --- |
| Resend **or** SMTP2GO account | Requires signup, billing, ToS acceptance |
| `XSTARZ_EMAIL_API_KEY` | Issued by that account; egress is blocked regardless |
| Registered domain | No domain is owned; inventing one would be a fake canonical URL |
| Sender verification | Provider-side DNS proof on a domain that must exist first |
| SPF / DKIM / DMARC | DNS records on the real domain. Deliverability is mostly this, not vendor choice |
| `CONVEX_SITE_URL` | Only exists once a deployment exists |
| Actual inbox delivery | Needs all of the above plus a real mailbox |
