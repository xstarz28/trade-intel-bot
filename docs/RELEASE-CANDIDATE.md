...
| In `main`'s tree? | **Yes** — `src/convex/auth/emailOtp.ts:29` at audit time. Phase 270: the module is retired and **removed from the tree entirely**; the credential identity remains the path+blob on record |
