...
|---|---|---|
| ~~`XSTARZ_EMAIL_TRANSPORT`~~ | **RETIRED (Phase 270)** | no longer an inventory variable; preflight does not read it |
| ~~`XSTARZ_EMAIL_API_KEY`~~ | **RETIRED (Phase 270)** | inert if set; removed from .env.example |
| ~~`XSTARZ_EMAIL_SENDER_ADDRESS`~~ | **RETIRED (Phase 270)** | inert if set |
...
## D — Real Email OTP Live Result — **RETIRED (Phase 270, 2026-09-24)**

**This gate item no longer exists.** The email-OTP provider was retired and
removed from `src/convex/auth.ts`; there is no email authentication path left
to configure or verify, so **EMAIL_AUTH_CONFIG_REQUIRED** is itself retired as
a launch-gate state (a missing configuration for a removed capability cannot
block anything). `XSTARZ_EMAIL_*` variables are inert if still set.

What remains true and testable from the original finding:

- The former code path was verified safe (no OTP in logs, no API key exposure,
  wrong/expired code fails, resend throttled via `otpLimiter.ts`); the durable
  limiter and `otpResendBuckets` table are retained frozen for data safety.
- No OTP value can ever be exposed in logs again: no OTP is generated.

**Result:** RETIRED — not required, not waivable as an exemption: the
capability it gated has been removed from the product.

---

## (History) former §D record

**Configured?** No. Transport `console` delivered nothing (forbidden in
production per the retired `emailDelivery.ts`), and no API key/sender existed.
Real delivery required a production transport + key + Xstarz-owned sender +
DNS; that configuration work became moot with the provider's retirement.
...
- ~~`XSTARZ_EMAIL_API_KEY`~~ (retired, Phase 270), `TWELVE_DATA_API_KEY`, `COINGLASS_API_KEY`, `ALPHA_VANTAGE_API_KEY`, `EIA_API_KEY`, `TICKATLAS_API_KEY`, `AUTH_GOOGLE_ID/SECRET`, `CONVEX_DEPLOY_KEY`, `XSTARZ_OWNER_PRINCIPALS` are server-only, presence-only, redacted via `redactSecrets`
...
- ~~Email OTP: OTP value never in logs, API key never exposed~~ — moot as of Phase 270 (the mechanism is gone, so there is nothing left to leak)
...
4. ~~**EMAIL_AUTH_CONFIG_REQUIRED**~~ **RETIRED (Phase 270):** email auth is removed; this blocker no longer exists on any path
...
5. ~~Set email ...~~ **RETIRED (Phase 270):** set no `XSTARZ_EMAIL_*` variable
