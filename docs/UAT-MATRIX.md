...
| ~~P3~~ | ~~Email OTP delivery works~~ | **RETIRED (Phase 270)** — email-OTP sign-in removed; no mailbox test can ever apply | §3 (retired) |
...
was used directly by `src/convex/auth/emailOtp.ts`, replacing the Phase 187
`makeFunctionReference` string workaround. **Phase 270:** the module was
retired and removed; this section documents a migration that no longer has a
host module — the limiter itself was never touched and remains retained-frozen.
...
> **Phase 270 addendum:** D1 (`mailbox delivery`) is retired — the harness records
> it terminally NOT_VERIFIED and there is no OTP mailbox anywhere in the
> mechanism. Rows 35g.5–35g.6 (mailbox/console-transport devices) can never run
> again: their BLOCKED state is permanent. D2–D10 continue to define the
> evidence on the anonymous mechanism.
...
> **Phase 270 addendum:** this gate is closed *by removal*, not by
> configuration. The email-OTP provider and its delivery stack were retired:
> every "PASS" row below now describes code that no longer exists (limiter and
> throttle tables remain, retained-frozen), and rows 18–19 are terminally
> unanswerable. Nothing here may be revived as a requirement.
...
> **Phase 270 addendum:** the gate this section gated is gone. The env-name
> protocol below remains valid practice for the surviving environment inventory
> (names-only, never values), but `XSTARZ_EMAIL_*` names no longer exist in the
> inventory, so the Case A / Case B decision is moot: DEV email smoke testing
> is impossible because there is no email path, not because env is absent.
...
> **Phase 270 addendum:** moot. There is no `XSTARZ_EMAIL_API_KEY` left to
> classify, disclose or gate; the defect class (contradictory classification of
> a variable) would now be caught for any new variable by the single-inventory
> `PRODUCTION_CONFIG_VARIABLES` model in `production-config.phase243`.
...
> **Phase 270 addendum:** the "external provisioning blocker" no longer gates
> anything: item 1–4 (domain, verified sender, DNS, mailbox) cannot be
> provisioned because the mechanism they would feed was removed. The recorded
> refusal (`EmailDeliveryError: ... not configured`) is superseded by stronger
> behaviour — the production-config surface does not evaluate email variables
> at all, and D1 in the harness is a permanent, explicitly-recorded
> NOT_VERIFIED (retired), not a provisioning gap.
