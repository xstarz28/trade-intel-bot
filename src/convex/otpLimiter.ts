 ...
 *
 * ## RETAINED FROZEN — Phase 270
 *
 * The email-OTP provider this limiter served was retired in Phase 270, so
 * this module no longer has a production caller: `consumeResendAllowance`
 * was only invoked by `auth/emailOtp.ts`, and `purgeExpiredBuckets` never
 * had one. The email-OTP UI was removed in Phase 269.
 *
 * This module and the `otpResendBuckets` table are RETAINED, not deleted:
 * the table holds durable anti-abuse state from the live deployment, and
 * schema/data removal is a separate data-lifecycle decision that Phase 270
 * deliberately does not make. Do NOT delete the table, do NOT add a cleanup
 * job, and do NOT re-wire this module into any provider.
 *
