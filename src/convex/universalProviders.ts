...
// ────────────────────────────────────────────────────────────────
// ALPHA VANTAGE INDEX (Phase 265)
// ────────────────────────────────────────────────────────────────

export const discoverAlphaVantageIndexes = action({
  args: {},
  handler: async (ctx): Promise<ProviderDiscoveryResult> => {
    await requireIdentity(ctx);
    const { discoverAlphaVantageIndexes } = await import("../lib/discovery/alpha-vantage-index-adapter");
    const readEnv = (name: string) => process.env[name];
    const transport = async (url: string, key: string) => {
      const separator = url.includes("?") ? "&" : "?";
      const fullUrl = url.includes("apikey=") ? url : `${url}${separator}apikey=${encodeURIComponent(key)}`;
    ...
    return discoverAlphaVantageIndexes(Date.now(), {
      transport,
      readEnv,
      catalogUrl: "https://www.alphavantage.co/query?function=INDEX_CATALOG",
    });
  },
});
...
export const discoverAllProviders = action({
  ...
  handler: async (ctx, args) => {
    ...
    const alphaVantagePromise = discoverAlphaVantageIndexes(now, {
      ...
      totalDiscovered: 0,
      catalogs: [],
      error: err instanceof Error ? err.message : "alpha-vantage index failed",
    }));
  ...
  handler: async (ctx, args) => {
    ...
    for (const [provider, items] of groups) {
      ...
      if (provider === "alpha-vantage") {
        // Phase 265 — INDEX_DATA daily/weekly/monthly, historical/delayed semantics
        const { fetchAlphaVantageIndexData } = await import(
          "../lib/discovery/alpha-vantage-index-adapter"
        );
        const avKey = process.env.ALPHA_VANTAGE_API_KEY ?? "";
        const transport = async (url: string, key: string) => {
          const sep = url.includes("?") ? "&" : "?";
          const full = url.includes("apikey=") ? url : `${url}${sep}apikey=${encodeURIComponent(key)}`;
          ...
          try {
            ...
            if (!result.success || result.candles.length === 0) {
              allResults.push({
                ...
                success: false,
                snapshot: null,
                error: result.error ?? "alpha-vantage index data unavailable",
                freshness: "DELAYED",
                timestampProvenance: "PROVIDER_OBSERVED",
              });
              continue;
            }
            // Convert last candle to snapshot-like structure for compatibility
            ...
            allResults.push({
              ...
              success: false,
              snapshot: null,
              error: msg || "alpha-vantage fetch failed",
            });
          }
        }
        continue;
