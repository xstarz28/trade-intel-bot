...
const OTP_SRC = read("src/convex/auth/emailOtp.ts");
...
describe("187.1 — the limiter state is durable and server-authoritative", () => {
  ...
  it("the auth provider consults the durable mutation, not the in-memory throttle", () => {
    expect(OTP_SRC).toContain("consumeResendAllowance");
    expect(OTP_SRC).toContain("runMutation");
    // The Phase 185 in-memory helpers must no longer be on the send path.
    expect(OTP_SRC).not.toContain("checkResendAllowed");
    expect(OTP_SRC).not.toContain("recordResend");
  });
...
describe("187.3 — limiter failure does not become unlimited access", () => {
  ...
  it("the auth provider turns a limiter failure into a refusal, not a send", async () => {
    // MUTATION-DRIVEN TEST. The original version of this assertion only read
    // the source text, and a mutation that made the catch block hand back
    // `{ allowed: true }` passed all 141 tests. That is the Phase 185 M4
    // lesson again: asserting a guard exists is not asserting it runs. This
    // version executes the real provider callback against a limiter that
    // throws, and additionally proves no email was sent.
    ...
    const { emailOtp } = await import("./auth/emailOtp");
    const provider = emailOtp as unknown as {
    ...
    // Capture stdout: the console transport writes there, so a send that
    // slipped through despite the limiter failure is directly observable.
    ...
    try {
      await provider.sendVerificationRequest(
        { identifier: "failclosed@example.com", token: "999999" },
        stub.ctx,
      );
    } catch (error) {
    ...
    expect(thrown!.message).toMatch(/Unable to send a verification code/i);
    // ...and crucially, nothing may have been delivered.
    expect(written.join("\n")).not.toContain("999999");
  });

  it("the refusal is ordered before delivery in the source", () => {
    const sendFn = OTP_SRC.slice(OTP_SRC.indexOf("async sendVerificationRequest"));
    expect(sendFn.indexOf("consumeResendAllowance")).toBeLessThan(
      sendFn.indexOf("sendXstarzVerificationEmail"),
    );
  });

  it("the failure message does not disclose that the limiter is the cause", () => {
    expect(OTP_SRC).toContain("Unable to send a verification code right now");
    const sendFn = OTP_SRC.slice(OTP_SRC.indexOf("async sendVerificationRequest"));
    // No internal mechanism naming in user-facing copy.
    expect(sendFn).not.toMatch(/throw new Error\([^)]*limiter/i);
    expect(sendFn).not.toMatch(/throw new Error\([^)]*database/i);
  });
  ...
  it("a delivery failure does not refund the allowance", async () => {
    ...
    // without limit. Asserted as documented intent plus behaviour.
    expect(OTP_SRC).toContain("NOT refunded");
    const stub = makeDurableLimiterStub();
    const t = 8_000_000;
...
describe("187.5 — failed attempts stay with Convex Auth, no second counter", () => {
  ...
  it("replay is prevented upstream by delete-on-use, not re-implemented here", () => {
    ...
    const code = OTP_SRC.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
    expect(code).not.toMatch(/verifyCode|consumeCode/);
  });
});
...
describe("187.7 — abuse diagnostics carry no sensitive data", () => {
  ...
  it("the user-facing throttle message reveals only a wait time", () => {
    expect(OTP_SRC).toMatch(/Try again in \$\{seconds\} seconds/);
    expect(OTP_SRC).not.toMatch(/\$\{email\}|\$\{identifier\}|\$\{token\}/);
  });

  it("no OTP, key, token or address appears in limiter or provider source", () => {
    for (const src of [LIMITER_SRC, OTP_SRC]) {
...
describe("187.8 — responses do not disclose account or throttle state", () => {
  ...
  it("throttle rejection does not distinguish an existing account", () => {
    // The message depends only on timing, never on whether a user exists.
    const sendFn = OTP_SRC.slice(OTP_SRC.indexOf("async sendVerificationRequest"));
    expect(sendFn).not.toMatch(/no such user|unknown account|not registered/i);
  });
});
...
describe("187.10 — Phase 185b/186 guarantees are untouched", () => {
  it("production still rejects the console transport and external issuers", () => {
    const email = read("src/convex/lib/emailDelivery.ts");
    const issuer = read("src/convex/lib/issuerPolicy.ts");
    expect(email).toContain("isProductionDeployment");
    expect(issuer).toContain("RETIRED_ISSUER_HOSTS");
  });

  it("no Freebuff OTP dependency returned to the runtime path", () => {
    expect(OTP_SRC).not.toMatch(/freebuff/i);
    expect(LIMITER_SRC).not.toMatch(/freebuff/i);
  });

  it("the OTP expiry policy is unchanged", () => {
    expect(OTP_SRC).toContain("OTP_EXPIRY_MINUTES = 10");
  });
});
