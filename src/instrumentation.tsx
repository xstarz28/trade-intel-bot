    ...
    >
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto border-zinc-700 bg-zinc-950 text-zinc-100 sm:max-w-lg">
        ...
        <DialogFooter className="gap-3 sm:items-center">
          {/*
            No link to the development platform here: this dialog is part of
            the shipped product, and a "open editor" action pointing at an
            internal tool is both broken and off-brand for end users.
          */}
  ...
  useEffect(() => {
    ...
    const handleRejection = async (event: PromiseRejectionEvent) => {
      try {
        ...
        console.error("[Xstarz Analysis runtime error]", normalizedError.error);
