...
describe("Phase264 9 — Alpha Vantage capability truth", () => {
  ...
  it("PROVIDER_API_SUPPORT vs CURRENT_ADAPTER_SUPPORT distinct (Phase265: now IMPLEMENTED)", () => {
    ...
    // Phase265: discoveryImplemented true via alpha-vantage-index-adapter
    expect(profile.discoveryImplemented).toBe(true);
    expect(profile.note ?? "").toContain("INDEX_CATALOG");
    expect(profile.note ?? "").toContain("alpha-vantage");
  });
  it("current adapter DOES implement INDEX_CATALOG/INDEX_DATA (Phase265)", () => {
  ...
  it("DXY not verified in INDEX_CATALOG (Phase265 determination)", () => {
    const profile = PROVIDER_DISCOVERY_PROFILES.find(p => p.provider==="alpha-vantage")!;
    expect(profile.note ?? "").toContain("DXY");
    expect((profile.note ?? "").toLowerCase()).toContain("dxy");
  });
...
describe("Phase264 12 — docs gap update", () => {
  ...
  it("docs reflect Alpha Vantage INDEX_CATALOG (Phase265 implemented)", () => {
    const src = readFileSync("docs/final-remaining-feature-gap.phase263.md", "utf8");
    expect(src).toContain("INDEX_CATALOG");
