...
describe("Phase258 1 — environment classification", () => {
  ...
  it("1.3 XSTARZ_DEPLOYMENT_ENV and XSTARZ_OWNER_PRINCIPALS classified; email transport retired (Phase 270)", () => {
    ...
    // Phase 270: XSTARZ_EMAIL_TRANSPORT is no longer a classified variable at
    // all — the email boundary was removed from the inventory model.
    expect(read("src/lib/deployment/production-config.ts")).not.toMatch(/XSTARZ_EMAIL_TRANSPORT/);
  });
});
...
describe("Phase258 8 — email OTP", () => {
  it("8.1 email OTP production requirement is retired (Phase 270)", () => {
    /* Pre-270 the pin asserted the email variables were required by
       production configuration. The email boundary was removed: production
       config now requires NOTHING email-shaped, and the module may only name
       the variables inside the retirement record. */
    const src = read("src/lib/deployment/production-config.ts");
    expect(src).not.toMatch(/name: "XSTARZ_EMAIL_/);
    expect(src).toMatch(/email authentication retired|RETIRED \(Phase 270\)|Phase 270/);
  });

  it("9.1 transport classification is retired with the module (Phase 270)", () => {
    // The console-vs-transport distinction lived in emailDelivery.ts, which is
    // gone: no classification exists to get wrong.
    expect(read("src/lib/deployment/production-config.ts")).not.toMatch(/emailDelivery/);
    expect(existsSync(join(ROOT, "src/convex/lib/emailDelivery.ts"))).toBe(false);
  });

  it("10.1 sender-ownership rule is retired: no sender exists (Phase 270)", () => {
    expect(existsSync(join(ROOT, "src/convex/lib/emailDelivery.ts"))).toBe(false);
  });
});
...
describe("Phase258 35 — auth", () => {
  ...
  it("36.1 email OTP regression is moot: the transport boundary is gone (Phase 270)", () => {
    expect(existsSync(join(ROOT, "src/convex/lib/emailDelivery.ts"))).toBe(false);
  });
});
...
describe("Phase258 41 — final matrix & gate", () => {
  ...
  it("58.1 auth session (Phase 270: OTP email provider removed)", () => {
