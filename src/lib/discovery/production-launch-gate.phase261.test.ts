...
describe("Phase261 B — Verify server environment", () => {
  it("B1 email transport retired: absent from env example, marked in doc (Phase 270)", () => {
    /* Pre-270 this pinned a `console` default in the env template. Phase 270
       retired the variable: the template must not offer it, and the gate doc
       records the retirement explicitly for traceability. */
    const envExample = read(".env.example");
    expect(envExample).not.toMatch(/^XSTARZ_EMAIL_TRANSPORT=/m);
    expect(envExample).toMatch(/email OTP retirement|Phase 270/);

    const checklist = read("docs/production-launch-gate.md");
    expect(checklist).toMatch(/XSTARZ_EMAIL_TRANSPORT/);
    expect(checklist).toMatch(/RETIRED \(Phase 270\)/);
  });

  it("B2 email API key and sender removed from template, marked retired in doc (Phase 270)", () => {
    const envExample = read(".env.example");
    expect(envExample).not.toMatch(/^XSTARZ_EMAIL_API_KEY=/m);
    expect(envExample).not.toMatch(/^XSTARZ_EMAIL_SENDER_ADDRESS=/m);
  ...
  it("B5 launch-gate records only statuses/retirement, never values (Phase 270)", () => {
    /* The original pin asserted PRESENT/MISSING markers from the email
       variables row. Those rows are retired; the durable property remains:
       the gate doc reports statuses (MISSING, RETIRED) and no secret-shaped
       values. */
    const checklist = read("docs/production-launch-gate.md");
    expect(checklist).toMatch(/MISSING/);
    expect(checklist).toMatch(/RETIRED \(Phase 270\)/);
...
describe("Phase261 D — Real Email OTP", () => {
  it("D1 EMAIL_AUTH_CONFIG_REQUIRED retired with the email OTP paths (Phase 270)", () => {
    const checklist = read("docs/production-launch-gate.md");
    expect(checklist).toMatch(/EMAIL_AUTH_CONFIG_REQUIRED.*RETIRED \(Phase 270\)/s);
  });

  it("D2 email OTP code path is removed; nothing can log an OTP (Phase 270)", () => {
    /* The prior no-logging pin applied to live files; both are retired. The
       stronger, structural pin: the files are gone and the auth surface has
       no OTP-bearing reference to leak. */
    expect(require("node:fs").existsSync(process.cwd() + "/src/convex/auth/emailOtp.ts")).toBe(false);
    expect(require("node:fs").existsSync(process.cwd() + "/src/convex/lib/emailDelivery.ts")).toBe(false);
    const auth = read("src/convex/auth.ts");
    expect(auth).not.toMatch(/emailOtp/);
    expect(auth).not.toMatch(/console\.log.*otp/i);
  });

  it("D3 wrong/expired-code handling documented as superseded by retirement (Phase 270)", () => {
    const checklist = read("docs/production-activation-checklist.md");
