...
describe("Phase259 4 — email OTP & Google OAuth", () => {
  it("4.1 email OTP config requirement is retired (Phase 270)", () => {
    /* The pre-270 pin asserted the three email variables were REQUIRED by
       production configuration. Phase 270 retired the email boundary: the
       legitimate state is that no email variable is declared at all and the
       module records the retirement. Guard rails: this may never regress to
       silently-requiring email configuration again. */
    const src = read("src/lib/deployment/production-config.ts");
    expect(src).not.toMatch(/name: "XSTARZ_EMAIL_/);
    expect(src).toMatch(/email authentication retired|Phase 270/);
  });
...
describe("Phase259 42 — security/secrets/live/historical/bounded/Gecko/CCXT/credential/license/network/malformed/rate-limit/retry/switching/refresh/race/stale/protected/readiness/diagnostics/identity/no-substitution/no-fake/UI/auth/email/OAuth/linking/duplicate/deployment/callback/fail-fast/regression/build/final", () => {
  ...
  it("70.1 email delivery module is removed (Phase 270)", () => {
    /* The console-vs-production delivery distinction lived in the retired
       emailDelivery module; with the module gone, production config must not
       reference it. */
    expect(exists("src/convex/lib/emailDelivery.ts")).toBe(false);
