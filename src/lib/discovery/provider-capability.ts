...
export const PROVIDER_DISCOVERY_PROFILES: ProviderDiscoveryProfile[] = [
  ...
  },
  {
    ...
    providerHasDiscoveryApi: true,
    discoverableAssetClasses: ["crypto"],
    note: "Dynamic CEX/DEX backbone via CCXT fetchMarkets(), exchanges discovered from ccxt.exchanges registry — EVENTUALLY_COMPLETE via cursor rotation maxExchanges=5 per cycle, eventual full 105 over 21 cycles.",
  },
  {
    ...
    providerHasDiscoveryApi: true,
    discoverableAssetClasses: ["crypto"],
    note: "DEX pairs via DexScreener public API search/?q= — BOUNDED_DISCOVERY, not complete DEX universe, pool identity = chain:dex:poolAddress, rotation of query partitions, source truth API response.",
  },
  {
    ...
    providerHasDiscoveryApi: true,
    discoverableAssetClasses: ["crypto"],
    note: "On-chain pools via GeckoTerminal API — EVENTUALLY_COMPLETE via cursor rotation: networks discovered dynamically via /networks, maxNetworks=3 per cycle rotating, pools paginated page param maxPages=2 per network, eventual full network coverage.",
  },
  {
    provider: "idx",
    discoveryImplemented: true,
  ...
  },
  {
    ...
    note: "Phase 265: Alpha Vantage INDEX_CATALOG (200+ indices) + INDEX_DATA daily/weekly/monthly OHLC implemented via src/lib/discovery/alpha-vantage-index-adapter.ts — provider=alpha-vantage providerInstrumentId exact native symbol byte-for-byte (e.g., SPX, DJI, NDX, VIX), assetClass indices subType index_cash, baseAsset symbol, quoteAsset USD, tradingState TRADING, capabilities ohlcv/quote, discoveredAt now, deterministic ordering, dedup via provider::providerInstrumentId, CREDENTIAL_REQUIRED/RATE_LIMITED/MALFORMED_RESPONSE, COMPLETE when catalog succeeds, no hardcoded list, catalog is source of truth. INDEX_DATA daily/weekly/monthly OHLC with timestamp provenance PROVIDER_OBSERVED, freshness DELAYED (historical), numerical validation, no synthetic candles. Premium plan required for INDEX_DATA per provider docs. DXY determination: search actual catalog — if DXY not returned, retain NOT_IMPLEMENTED, no proxy substitution.",
  },
  {
    provider: "coinglass",
    discoveryImplemented: true,
