...
describe("Phase256 20 — auth", () => {
  it("20.1 Email OTP is retired (Phase 270 amendment)", () => {
    /* The old pin asserted the path "existed" (still wired, per Phase 269's
       deferred retirement). Phase 270 retired it: now the pin inverts and is
       strictly stronger — the auth surface contains no trace of it. */
    const src = read("src/convex/auth.ts");
    expect(src).not.toMatch(/emailOtp/i);
    expect(src).toMatch(/providers:\s*\[\s*Anonymous,\s*googleProvider\s*\]/);
  });
