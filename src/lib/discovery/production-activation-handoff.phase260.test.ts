...
describe("Phase260 5 — email configuration", () => {
  it("5.1 email configuration is retired in checklist and runtime (Phase 270)", () => {
    /* Pre-270 this pinned the DEV_CONSOLE_ONLY-vs-PRODUCTION_READY split and
       the transport inventory. Phase 270 retired the email boundary: the
       checklist now records the retirement explicitly (traceability kept) and
       the runtime declares no email variable. The guard net did not weaken —
       it now states there is NO email path whose classification could drift. */
    const checklist = read("docs/production-activation-checklist.md");
    expect(checklist).toMatch(/Email Auth Activation — RETIRED \(Phase 270\)/);
    expect(checklist).toMatch(/EMAIL_AUTH_CONFIG_REQUIRED.*retired/s);
    expect(checklist).toMatch(/Email OTP.*removed and its delivery stack deleted/s);

    const prodConfig = read("src/lib/deployment/production-config.ts");
    expect(prodConfig).not.toMatch(/name: "XSTARZ_EMAIL_/);
    expect(prodConfig).toMatch(/email authentication retired|Phase 270/);
  });

  it("5.2 email transport split is moot: the module is gone (Phase 270)", () => {
    expect(existsSync(join(ROOT, "src/convex/lib/emailDelivery.ts"))).toBe(false);
  });
});
...
describe("Phase260 8 — no OTP", () => {
  ...
  it("8.2 email OTP implementation is removed; auth surface cannot log an OTP (Phase 270)", () => {
    /* The old no-logging pin applied to a live provider file. The file is
       retired; the stronger assertion is structural: the auth surface has no
       OTP-bearing code path at all. */
    expect(existsSync(join(ROOT, "src/convex/auth/emailOtp.ts"))).toBe(false);
    const auth = read("src/convex/auth.ts");
    expect(auth).not.toContain("emailOtp");
    expect(auth).not.toMatch(/console\.log.*otp/i);
    expect(auth).not.toMatch(/console\.log.*code/i);
  });
});
...
describe("Phase260 24 — Convex config", () => {
  it("24.1 Convex config inventory includes required vars", () => {
    ...
    // Phase 270: email transport is no longer part of the required inventory.
    expect(src).not.toMatch(/name: "XSTARZ_EMAIL_TRANSPORT/);
  });
...
describe("Phase260 30 — email OTP", () => {
  it("30.1 email OTP console/prod distinction is retired (Phase 270)", () => {
    expect(existsSync(join(ROOT, "src/convex/lib/emailDelivery.ts"))).toBe(false);

    const checklist = read("docs/production-activation-checklist.md");
    expect(checklist).toMatch(/retired|RETIRED/);
    expect(checklist).toMatch(/EMAIL_AUTH_CONFIG_REQUIRED/);
  });

  it("30.2 no email OTP code path exists to leak an API key or OTP (Phase 270)", () => {
    expect(existsSync(join(ROOT, "src/convex/auth/emailOtp.ts"))).toBe(false);
    const auth = read("src/convex/auth.ts");
    expect(auth).not.toContain("emailOtp");
    expect(auth).not.toMatch(/console\.log.*apiKey/i);
    expect(auth).not.toMatch(/console\.log.*OTP/i);
  });
});
