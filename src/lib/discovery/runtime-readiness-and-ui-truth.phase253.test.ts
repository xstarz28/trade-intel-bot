...
describe("Phase253 10 — OAuth no-OTP", () => {
  ...
  it("no OTP redirect after Google (Phase 270: no email OTP at all)", () => {
    const src = read("src/convex/auth.ts");
    /* The old pin asserted the OTP provider remained wired but unreachable
       after OAuth. Phase 270 retired it: the no-redirect guarantee is now
       structural — nothing named emailOtp exists to redirect to. */
