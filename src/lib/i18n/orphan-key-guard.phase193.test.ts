 ...
 *
 * Phase 269: 226 -> 242, the deliberate-raise path this ratchet exists to
 * force. Email OTP was removed from the normal visible sign-in flow, so the
 * 17 `auth.*` keys that only the email-entry/OTP step rendered lost their
 * consumer. The vocabulary is RETAINED, not deleted: the email-OTP backend
 * provider and its delivery infrastructure are intact, all nine locales are
 * pinned to the same key count, and retirement of that infrastructure is a
 * separate decision. Every one of the 17 keys is individually justified in
 * INTENTIONAL_ALLOWLIST below — this raise contains zero slack and zero
 * hidden orphans.
 *
 ...
 */
/* Phase 270: the Phase-269 OTP copy block retired with the email-OTP backend
 * — the keys it allowed are deleted from every locale, so an allowlist entry
 * would now fail the "allowlisted key exists" check. The block is kept here,
 * struck, as the record of why the 17 keys no longer appear. */
...
const INTENTIONAL_ALLOWLIST: Record<string, { reason: string; surface: string }> = {
  ...
  // ── retained backend delivery infrastructure (17 keys, no slack).
};
...
describe("193 — the orphan-key guard is real, not vacuous", () => {
  ...
  it("known-rendered keys are never reported as unreferenced", () => {
    // Positive control with keys proven to render. Phase 269 replaced
    // auth.sendFailed / auth.codeIncorrect (their consumer, the email-OTP
    // step, was removed — they are now explicitly allowlisted above) with
    // the keys the normal Google-only surface actually renders.
