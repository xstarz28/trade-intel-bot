...
describe("exact leaf-key parity across all 9 locales", () => {
  ...
  it("EN is the canonical structural reference with 1256 leaves", () => {
    ...
    // Phase 270: 17 email-OTP keys retired (1229 → 1212).
    // Phase 274: 13 fundamental-assessment keys added to `analysisResult`
...
describe("ZH (Simplified Chinese) — explicit verification", () => {
  ...
  it("zh has all 1256 canonical keys with non-empty values", () => {
    const zhLeaves = collectLeaves(zh);
    expect(zhLeaves.length).toBe(1256);
