...
describe("exact leaf-key parity across all 9 locales", () => {
  ...
  it("EN is the canonical structural reference with 1005 leaves", () => {
...
describe("ZH (Simplified Chinese) — explicit verification", () => {
  ...
  it("zh has all 1005 canonical keys with non-empty values", () => {
    const zhLeaves = collectLeaves(zh);
    expect(zhLeaves.length).toBe(1005);
