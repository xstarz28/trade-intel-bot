...
describe("exact leaf-key parity across all 9 locales", () => {
  ...
  it("EN is the canonical structural reference with 1035 leaves", () => {
...
describe("ZH (Simplified Chinese) — explicit verification", () => {
  ...
  it("zh has all 1035 canonical keys with non-empty values", () => {
    const zhLeaves = collectLeaves(zh);
    expect(zhLeaves.length).toBe(1035);
