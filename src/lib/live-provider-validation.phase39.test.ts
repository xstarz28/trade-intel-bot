 ...
 *
 * This test file exercises the PURE normalizer/parser modules, plus the
 * graceful-failure behavior of keyed providers (Twelve Data, Alpha Vantage,
 * CoinGlass, EIA, TickAtlas).
 *
 * Phase 237: the sections that hit PUBLIC provider endpoints (Treasury, CFTC,
 * OKX, CoinGecko) moved to `live-provider-endpoints.live.test.ts`. Everything
 * left here is hermetic — it either pure-logic or explicitly about a missing
 * credential — so it holds with the network guard installed.
 */
