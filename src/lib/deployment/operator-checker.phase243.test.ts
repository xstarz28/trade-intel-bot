...
describe("243 — the operator checker, run as an operator runs it", () => {
  ...
  it("refuses the console transport for production", () => {
    const result = run([
      "--config",
    ...
    expect(result.status).toBe(1);
    expect(outcomeOf(result)).toBe("FORBIDDEN_FALLBACK");
    expect(result.stdout).toMatch(/delivers nothing|non-delivering/);
  });
  ...
  it("emits machine-readable JSON that carries the same outcome", () => {
    ...
    expect(parsed.email.senderVerified).toBe(false);
  ...
  it("15. no secret value reaches stdout or stderr, in any outcome", () => {
    const cases: [string, Record<string, string>][] = [
      ["ok", complete()],
      ["console", { ...complete(), XSTARZ_EMAIL_TRANSPORT: "console" }],
    ...
    for (const [name, config] of cases) {
      ...
      const secrets = Object.entries(config)
        ...
        .map(([, value]) => value);
      expect(secrets.length, name).toBeGreaterThan(5);
