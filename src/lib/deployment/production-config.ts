 ...
 *    `issuerPolicy.ts` and the provider credential specs from the code that runs
 *    in production. A second copy of the rules would drift and produce a
 *    confident, wrong acceptance — the exact failure this phase exists to prevent.
 *
 * Phase 270 (email retirement): the email-OTP provider was retired, so there is
 * no email authentication path left to configure. XSTARZ_EMAIL_* is no longer in
 * the inventory — nothing is read, required, validated or accepted for email
 * here, and a still-configured XSTARZ_EMAIL_* variable reports as `unexpected`
 * (production-shaped, not declared) rather than load-bearing.
...
export interface ConfigVariable {
  ...
  shape?: "https-url" | "deployment-identity" | "issuer-url";
}
...
export function evaluateProductionConfiguration(
  ...
  /* 5. Email: retired in Phase 270. The email-OTP provider is gone, so no
     email transport, credential or sender is required, validated or accepted
     for authentication. A still-configured XSTARZ_EMAIL_* variable is not
     load-bearing: it was already counted under `unexpected` above. The
     retired identity-protection property survives on the issuer side via
     RETIRED_ISSUER_HOSTS in issuerPolicy.ts. */
  say("email: authentication path retired (Phase 270); no email configuration is required or accepted");
