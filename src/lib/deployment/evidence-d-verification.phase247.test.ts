  ...
  it("51. the phase 242 admission stays authoritative and refuses without a real package", () => {
    ...
    const accepted = evaluateReleaseAdmission({
      source: memorySource({
        ...
        /* Phase 270: the PRODUCTION_EMAIL_TRANSPORT proof path retired with the
           email-OTP provider — release admission no longer asks for it, so this
           synthetic tree files no such proof either. */
        [PROOF_PATHS.evidenceD]: JSON.stringify(pkg()),
      }),
      now: NOW,
      commit: CANDIDATE_COMMIT,
