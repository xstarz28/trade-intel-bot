...
describe("Phase 209 — fixture output can never be presented as evidence", () => {
  it("stamps fixture=true, the FIXTURE class, and a synthetic deployment identity", async () => {
    /* Phase 270: no --auth flag — anonymous is the only mechanism. */
    const r = await runAgainstFixture("complete", ["--json"]);
  ...
  it("keeps productionEvidence false at the maximum-achievement run (9/10; D1 is terminally NOT_VERIFIED)", async () => {
    const r = await runAgainstFixture("complete", ["--json"]);
    const report = parseJson(r.stdout);
    /* Phase 270 hardened this property: 10/10 is now impossible (D1 retired,
       so it records NOT_VERIFIED forever). Every observable check passes and
       the run is STILL not evidence — neither a verdict nor a class. */
  ...
  it("never emits DEV_VERIFIED for a fixture run, whatever the env says", async () => {
    ...
    const r = spawnSync(
      "node",
      [HARNESS, "--fixture", `http://127.0.0.1:${port}`, "--json"],
      {
        cwd: root,
        encoding: "utf8",
        timeout: 60_000,
...
describe("Phase 209 — exit-code contract across fixture scenarios", () => {
  it("never exits 0 for a fixture, even when every observable check passes", async () => {
    /* Phase 270: "exits 0 only when everything passed" became stronger — D1's
       retired NOT_VERIFIED is terminal, so the exit-0 run is now impossible
       for the fixture, not just unearned. */
    const r = await runAgainstFixture("complete", ["--json"]);
    const report = parseJson(r.stdout);
