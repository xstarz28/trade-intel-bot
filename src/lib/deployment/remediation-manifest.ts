...
export const EXPOSED_CREDENTIAL: CredentialIdentity = {
  ...
  secretLength: 33,
  /* Phase 270: the path no longer exists in the tree — the email-OTP module
   * was retired and removed outright. The path stays recorded because, with
   * the blob fingerprint, it is the historical identity of the leak: A2's
   * history rewrite answers exactly (path, blob), not whatever file happens
   * to sit at the location today. At HEAD "clean" means "absent". */
