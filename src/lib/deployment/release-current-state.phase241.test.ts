...
describe("241 — the reader refuses to promote a claim", () => {
  ...
  it("a proof file with the wrong environment is refused", () => {
    const state = deriveCurrentReleaseState(
      memorySource({
        [PROOF_PATHS.refInventory]: inventory,
        [PROOF_PATHS.emailDelivery]: JSON.stringify({
          verified: true,
          source: "external-verification",
          environment: "development",
          observedAt: Date.now() - 1000,
          detail: "delivered through the development transport",
        }),
      }),
    );

    const email = state.verdict.prerequisites.find((p) => p.id === "PRODUCTION_EMAIL_TRANSPORT");
    expect(email?.state).toBe("UNVERIFIED");
    expect(email?.reasons.join(" ")).toContain("wrong environment");
    expect(state.verdict.verdict).toBe("NOT READY");
  });
