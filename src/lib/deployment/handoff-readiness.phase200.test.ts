...
describe("Phase 200 — the Evidence D harness cannot be satisfied by a substitute", () => {
  ...
  it("records D1 as retired (Phase 270), never as a delivery inferred from a 200", () => {
    const source = readFileSync(HARNESS, "utf8");
    // The mailbox path no longer exists: D1 is a preserved retired record —
    // terminal NOT_VERIFIED, mechanism "retired", and no email input.
    expect(source).toMatch(/RETIRED \(Phase 270\)/);
    expect(source).toMatch(/OTP mailbox delivery is retired \(Phase 270\)/);
    expect(source).toMatch(/mechanism: "retired"/);
    expect(source).not.toMatch(/EVIDENCE_D_EMAIL=/);
    expect(source).not.toMatch(/maskEmail/);
  });
