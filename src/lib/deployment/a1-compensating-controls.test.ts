 ...
 *
 * Phase 270: the observed control surface evolved with the retirement of the
 * email-OTP provider. The strongest compensating control is that the OTP
 * email path no longer exists; the observer reads `src/convex/auth.ts`,
 * `src/convex/lib/issuerPolicy.ts` and the frozen `src/convex/otpLimiter.ts`,
 * and treats any reappearance of the retired modules as a finding.
 */
...
function realControlFiles(): Record<string, string> {
  return {
    "src/convex/auth.ts": read("src/convex/auth.ts"),
    "src/convex/lib/issuerPolicy.ts": read("src/convex/lib/issuerPolicy.ts"),
    "src/convex/otpLimiter.ts": read("src/convex/otpLimiter.ts"),
  };
}
...
describe("A1 compensating-controls — runtime observer", () => {
  ...
  it("a restored Freebuff OTP call is not a compensating control", () => {
    // The retired world: a Freebuff call could only live on the retained auth
    // surface. Simulating it there must fail the observation.
    const files = realControlFiles();
    files["src/convex/auth.ts"] += "\n// regression\nfetch(\"https://auth.freebuff.app/send_otp\");\n";
  ...
  it("a reappearance of the retired email modules is a finding, not a control", () => {
    const files = realControlFiles();
    files["src/convex/auth/emailOtp.ts"] = "export const emailOtp = {};";
    files["src/convex/lib/emailDelivery.ts"] = "export const FORBIDDEN_DELIVERY_HOSTS = [];";
    const obs = observeA1RuntimeControls(files);
    expect(compensatingControlsProven(obs)).toBe(false);
    expect(obs.usesXstarzEmailAbstraction).toBe(false);
    expect(obs.productionEmailFailsClosed).toBe(false);
    expect(obs.problems.join(" ")).toMatch(/reappeared/);
  });

  it("a re-registered email-OTP provider is a finding, not a control", () => {
    const files = realControlFiles();
    files["src/convex/auth.ts"] = files["src/convex/auth.ts"].replace(
      "providers: [Anonymous, googleProvider]",
      "providers: [Anonymous, googleProvider, emailOtp]",
    );
    const obs = observeA1RuntimeControls(files);
    expect(compensatingControlsProven(obs)).toBe(false);
    expect(obs.noFreebuffOtpFallback).toBe(false);
    expect(obs.problems.join(" ")).toMatch(/must keep email-OTP retired/);
  });

  it("a frozen limiter that loses its marker is a finding", () => {
    const files = realControlFiles();
    files["src/convex/otpLimiter.ts"] = files["src/convex/otpLimiter.ts"].replace(
      "RETAINED FROZEN",
      "back on the active path",
    );
    const obs = observeA1RuntimeControls(files);
    expect(compensatingControlsProven(obs)).toBe(false);
    expect(obs.productionEmailFailsClosed).toBe(false);
    expect(obs.problems.join(" ")).toMatch(/RETAINED FROZEN/);
  });
