...
describe("244 — the manifest is a measurement, not a memory", () => {
  ...
  it("states that the leaked path is clean today while the history is not", () => {
    // Phase 270: the strongest form of clean is absent — the whole module
    // retired, so there is no file that could still carry the leak.
    if (existsSync(resolve(root, REMEDIATION_MANIFEST.credential.path))) {
