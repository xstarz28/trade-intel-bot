...
describe("Phase 203 — a development run is never production evidence", () => {
  ...
  it("refuses anonymous sign-in as production evidence", () => {
    ...
    expect(run.exitCode).toBe(2);
    expect(JSON.parse(run.stdout).reason).toMatch(/only session mechanism is anonymous development sign-in/i);
  });
});
...
describe("Phase 235 — an answer that is not a working deployment is not authentication evidence", () => {
  ...
  it("only passes D3 when the service actually answered with a refusal", () => {
    ...
    for (const [mode, withheld] of [
      ...
      // One passing check is still not a passed gate. Phase 270: the stubbed
      // 401 answers the anonymous sign-in itself, so D2 records FAIL (an
      // honest authentication refusal) — exit 1/FAILED rather than the old
      // BLOCKED-shaped INCOMPLETE. D1 stays NOT_VERIFIED (retired record).
      expect(run.exitCode, mode).toBe(1);
      expect(report.evidenceD, mode).toBe("FAILED");
...
describe("Phase 203 — partial evidence is never reported as complete", () => {
  ...
  it("records D1 as retired (Phase 270), never as a transport result", () => {
    // The OTP mailbox path no longer exists: D1 is a preserved retired record,
    // not something mail transport configuration could influence.
    expect(harnessSource).toMatch(/OTP mailbox delivery is retired \(Phase 270\)/);
    expect(harnessSource).toMatch(/"D1",\s*\n?\s*"NOT_VERIFIED"/);
    expect(harnessSource).not.toContain("XSTARZ_EMAIL_TRANSPORT");
    expect(harnessSource).not.toContain("EVIDENCE_D_EMAIL");
  });

  it("marks an already-used identity NOT_VERIFIED for D4 rather than FAIL (and names no mailbox)", () => {
    ...
    // Phase 270: "use a new mailbox" would direct the operator at a retired
    // mechanism; the hint must name the supported one.
    const d4Hint = /this identity has already consumed[\s\S]*?e0,\s*\);/.exec(harnessSource);
    expect(d4Hint?.[0] ?? "").not.toMatch(/mailbox/i);
    expect(d4Hint?.[0] ?? "").toMatch(/--auth anonymous/);
  });
...
describe("Phase 203 — the harness never emits a secret", () => {
  it("withholds session tokens, and takes no mailbox input at all", () => {
    // Phase 270: maskEmail retired with the mailbox it masked — the stronger
    // invariant is that no address enters the harness in the first place.
    expect(harnessSource).not.toContain("maskEmail");
    expect(harnessSource).not.toMatch(/EVIDENCE_D_EMAIL/);
  ...
  it("the operator documentation carries no credential values", () => {
    ...
    // Phase 270: the mailbox variable is presented as RETIRED only — never as
    // a variable an operator should set.
    expect(text).toMatch(/EVIDENCE_D_EMAIL.*RETIRED \(Phase 270\)/s);
    expect(text).not.toMatch(/^\s*export EVIDENCE_D_EMAIL/m);
  });
});
...
describe("Phase 204 — D4-D7 run on one identity, and a rejected session cannot pass", () => {
  it("acquires exactly one token and reuses it for every authenticated call", () => {
    ...
    // Phase 270: one mechanism remains (anonymous), so one assignment remains.
    expect(assignments.length, "token must be assigned exactly once").toBe(1);
