...
describe("243 — no credential literals in the configuration surfaces", () => {
  ...
  it("the template keeps every credential empty or explicitly local", () => {
    ...
    // Phase 270: the retired email variables are gone from the template
    // itself — they appear only in a comment that records the retirement.
    expect(template.match(/^XSTARZ_EMAIL_[A-Z_]*=.*/gm)).toBeNull();
    expect(template).toMatch(/RETIRED in Phase 270/);
  });
