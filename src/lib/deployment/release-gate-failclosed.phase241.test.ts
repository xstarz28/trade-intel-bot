 ...
 *
 * (Phase 270: the five-prerequisite manifest lost
 * `PRODUCTION_EMAIL_TRANSPORT` — the capability it proved no longer exists.
 * The gate-and-blocker tests for that id are replaced by retirement
 * assertions; the id stays reserved and any future evidence claiming it is
 * an unrecognised claim.)
 *
...
describe("241 — the one shape that may produce READY", () => {
  it("1. every mandatory prerequisite VERIFIED => READY", () => {
    ...
    // Only the four mandatory prerequisites decide the verdict; nothing else
    // is in the manifest that could gate a release by existing.
    expect(verdict.prerequisites.filter((p) => p.mandatory).map((p) => p.id).sort()).toEqual(
      [A1, A2, DEPLOY, EVIDENCE_D].sort(),
    );
  });
  ...
  it("5. one CONTRADICTORY prerequisite => NOT READY, and the pass is not preferred", () => {
    const records = [
      ...allVerified(),
      {
        ...
        detail: "one affected ref still serves the blob at tip",
      },
    ];
    ...
    expect(stateOf(records, A2)).toBe("CONTRADICTORY");
    expect(verdict.blockers).toContain(A2);
  });
});
...
describe("241 — deployment, email and Evidence D gates", () => {
  ...
  it("9. (retired, Phase 270) the email-transport prerequisite no longer gates anything", () => {
    // There is no EMAIL row to be unverified: the manifest retired the id
    // because the capability it proved was deleted, not merely unconfigured.
    expect(RELEASE_PREREQUISITES.some((p) => p.id === EMAIL)).toBe(false);
    const verdict = evaluate(allVerified());
    expect(verdict.prerequisites.some((p) => p.id === EMAIL)).toBe(false);
  });

  it("9b. (retired, Phase 270) a stray PRODUCTION_EMAIL_TRANSPORT record is an unrecognised claim", () => {
    // A proof filed for the retired id must not resurrect the gate: it lands
    // on `unrecognised` (and `unrecognisedVerified` when it claims VERIFIED),
    // exactly like any other id outside the manifest, and the verdict fails.
    const verdict = evaluate([...allVerified(), proof(EMAIL)]);
    expect(verdict.verdict).toBe("NOT READY");
    expect(verdict.unrecognised).toContain(EMAIL);
    expect(verdict.blockers.join(" ")).toContain("unrecognised evidence claiming VERIFIED");
    expect(verdict.prerequisites.some((p) => p.id === EMAIL)).toBe(false);
  });
...
describe("241 — malformed, unknown and failing input", () => {
  it("16. malformed evidence fails closed and never throws", () => {
    const malformed: EvidenceRecord[] = [
      ...
      proof(EVIDENCE_D, { source: "hope" as unknown as EvidenceRecord["source"] }),
      null as unknown as EvidenceRecord,
      /* Phase 270: PRODUCTION_EMAIL_TRANSPORT retired; a record for the id is
         now exercised in 17d-style unrecognised-claim form (see 9b). */
    ];
  ...
  it("17b. an empty evidence object is NOT READY, not vacuously READY", () => {
    ...
    expect(verdict.verdict).toBe("NOT READY");
    expect([...verdict.blockers].sort()).toEqual([A1, A2, DEPLOY, EVIDENCE_D].sort());
  });
  ...
  it("17g. owner-risk-acceptance cannot satisfy a prerequisite that does not list it", () => {
    const records = allVerified().map((r) =>
      r.prerequisite === EVIDENCE_D
        ? proof(EVIDENCE_D, { source: "owner-risk-acceptance", detail: "we accept the coverage risk" })
        : r,
    );
    const verdict = evaluate(records);
    expect(verdict.verdict).toBe("NOT READY");
    expect(stateOf(records, EVIDENCE_D)).not.toBe("VERIFIED");
    const reasons = verdict.prerequisites.find((p) => p.id === EVIDENCE_D)?.reasons ?? [];
...
describe("241 — the aggregate is deterministic and order-independent", () => {
  ...
  it("19b. every blocker state is one of the four non-passing states", () => {
    ...
    const verdict = evaluate([
      ...allVerified().filter((r) => ![A1, A2, EVIDENCE_D].includes(r.prerequisite)),
      proof(A2, { observedAt: NOW - 40 * DAY }),
