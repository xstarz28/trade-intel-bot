...
import { readEmailDeliveryConfig, EmailDeliveryError } from "@/convex/lib/emailDelivery";
...
describe("Phase 217 — the deployment fails closed in the confirmed DEV state", () => {
  it("refuses to send when transport, key and sender are all absent", () => {
    let thrown: unknown = null;
    try {
      readEmailDeliveryConfig((k: string) => CONFIRMED_DEV_ENV[k]);
    } catch (e) {
      thrown = e;
    }
    expect(thrown).toBeInstanceOf(EmailDeliveryError);
    expect((thrown as EmailDeliveryError).reason).toBe("not_configured");
  });

  it("names the variables the operator must provision", () => {
    try {
      readEmailDeliveryConfig((k: string) => CONFIRMED_DEV_ENV[k]);
      expect.unreachable("configuration must be refused");
    } catch (e) {
      const message = (e as Error).message;
      expect(message).toContain("XSTARZ_EMAIL_API_KEY");
      expect(message).toContain("XSTARZ_EMAIL_SENDER_ADDRESS");
    }
  });

  it("defaults an absent transport to a REAL transport, never console", () => {
    // If absence defaulted to `console`, sign-in would appear to work while
    // delivering nothing — the single most dangerous misconfiguration.
    let thrown: unknown = null;
    try {
      readEmailDeliveryConfig((k: string) => CONFIRMED_DEV_ENV[k]);
    } catch (e) {
    ...
    // A console default would have RESOLVED rather than thrown.
    expect(thrown, "absent transport must not silently resolve to console").toBeInstanceOf(
      EmailDeliveryError,
    );
  });
});
