...
function completeConfig(): Record<string, string> {
  return {
    ...
    /* Phase 270: a complete configuration contains no XSTARZ_EMAIL_* names —
     * the inventory retired them. The one remaining secret inventory entry
     * (owner principals) is set synthetically. */
    XSTARZ_OWNER_PRINCIPALS: ["owner243", "@", "xstarz.example"].join(""),
...
describe("243 — the inventory", () => {
  it("declares names, consumers and scopes, and never a value", () => {
    expect(PRODUCTION_CONFIG_VARIABLES.length).toBe(6);
    // Phase 270: the email variables retired out of the inventory entirely.
...
describe("243 — a complete configuration, and what it does not mean", () => {
  ...
  it("13. configuration presence is not verification, in every surface", () => {
    ...
    // Phase 270: email.senderVerified retired with the email boundary; the
    // deployment verification marker and the production claim stay false.
...
describe("243 — missing and malformed configuration fail closed", () => {
  it("2. a missing required variable is reported by name", () => {
    for (const name of ["CONVEX_SITE_URL", "VITE_CONVEX_URL", "CONVEX_DEPLOYMENT"]) {
  ...
  it("3. malformed values are refused with a reason and never echoed", () => {
    ...
    // is not a credential, and a refusal that names the problem is the sentence
    // that fixes it). Secrets are the invariant, and they are proven above.
    /* Phase 270: the echo test rides the one remaining secret variable —
     * a retired-codec email key would only prove the unexpected-variable path. */
    const secret = ["e243", "aabbccddeeff0011"].join("-");
    const leaked = evaluate({ XSTARZ_OWNER_PRINCIPALS: secret, CONVEX_SITE_URL: "http://xstarz.example" });
  ...
  it("18b. a documentation-style example is refused for production", () => {
    ...
    // Phase 270: .env.example no longer mentions email transport at all —
    // assert it rather than feed it here.
    const envExample = readFileSync(".env.example", "utf8");
    expect(envExample).not.toMatch(/^XSTARZ_EMAIL_/m);
...
describe("243 — environment and identity separation", () => {
  ...
  it("7. fixture and placeholder values cannot become production configuration", () => {
    for (const value of ["test", "fixture", "dummy-key", "abc123", "sk_test_abcdefgh", "REPLACE_WITH_KEY", "<your-key>"]) {
      const report = evaluate({ TWELVE_DATA_API_KEY: value });
  ...
  it("reports unexpected production-shaped variables without inventing rules for them", () => {
    ...
    // Phase 270: the retired email variables join the same reporting path —
    // present, but never load-bearing.
    const legacy = evaluate({ XSTARZ_SOMETHING_NEW: "yes", CONVEX_EXTRA_VAR: "also", XSTARZ_EMAIL_TRANSPORT: "resend" });
    expect(legacy.unexpected).toEqual(["CONVEX_EXTRA_VAR", "XSTARZ_EMAIL_TRANSPORT", "XSTARZ_SOMETHING_NEW"]);
    expect(report.outcome).toBe("READY_FOR_CONFIGURATION");
    expect(report.diagnostics.join(" ")).toMatch(/unexpected production-shaped variables/);
  });
});

describe("243 — the email boundary (retired, Phase 270)", () => {
  it("8. the console-transport concept is gone, stronger than forbidden: there is no email boundary at all", () => {
    const report = evaluate({ XSTARZ_EMAIL_TRANSPORT: "console" });
    // The value is inert: it cannot configure a transport, because no
    // transport exists. It reports as unexpected production-shaped config.
    expect(report.unexpected).toContain("XSTARZ_EMAIL_TRANSPORT");
    expect(Object.keys(report as unknown as Record<string, unknown>)).not.toContain("email");
  });

  it("9. a retired email credential or sender name changes no outcome", () => {
    const report = evaluate({
      XSTARZ_EMAIL_TRANSPORT: "not-a-transport",
      XSTARZ_EMAIL_API_KEY: EMAIL_SECRET,
      XSTARZ_EMAIL_SENDER_ADDRESS: "no-reply@auth.freebuff.app",
    });
    // None of them are inventory entries: they cannot be malformed or
    // forbidden — only unexpected, and never a credential the report keeps.
    expect(report.malformed).toEqual([]);
    expect(report.forbidden).toEqual([]);
    ...
    expect(JSON.stringify(report)).not.toContain(EMAIL_SECRET);
  });

  it("10. a Freebuff or VLY issuer is still refused (issuer boundary unchanged)", () => {
  ...
  it("10c. the retirement is reported in the diagnostics, not silently absent", () => {
    const report = evaluate();
    expect(report.diagnostics.join(" ")).toMatch(/retired/i);
  });
});
