...
export function observeA1RuntimeControls(
  ...
  const auth = files["src/convex/auth.ts"] ?? "";
  const issuerPolicy = files["src/convex/lib/issuerPolicy.ts"] ?? "";
  const otpLimiter = files["src/convex/otpLimiter.ts"] ?? "";
  const problems: string[] = [];

  if (auth.length === 0) problems.push("src/convex/auth.ts is missing");
  if (issuerPolicy.length === 0) problems.push("src/convex/lib/issuerPolicy.ts is missing");
  if (otpLimiter.length === 0) problems.push("src/convex/otpLimiter.ts is missing");

  /* Phase 270 — the email-OTP provider and its delivery stack are retired.
     That absence is the strongest form of the compensating control: there is
     no OTP email path left that could ever touch the leaked platform
     credential. The observation field names are retained so the observation
     contract does not drift; each now reads from the retired world:
       - usesXstarzEmailAbstraction   → no email abstraction exists at all:
                                        the provider must not be registered
                                        and both retired files must be absent
       - productionEmailFailsClosed   → nothing can deliver: the delivery
                                        module stays absent and the frozen
                                        limiter keeps its marker */

  const namesFreebuffHost = /auth\.freebuff\.app/.test(auth);
  const namesSendOtp = /send_otp/.test(auth);
  if (namesFreebuffHost) problems.push("auth.ts names auth.freebuff.app");
  if (namesSendOtp) problems.push("auth.ts names the retired send_otp path");
  const neverCalls = auth.length > 0 && !namesFreebuffHost && !namesSendOtp;

  const otpProviderRetired =
    auth.length > 0 &&
    !/import\s*\{\s*emailOtp\s*\}\s*from/.test(auth) &&
    /providers:\s*\[\s*Anonymous\s*,\s*googleProvider\s*\]/.test(auth);
  if (auth.length > 0 && !otpProviderRetired) {
    problems.push(
      "auth.ts must keep email-OTP retired: providers are exactly [Anonymous, googleProvider] and no emailOtp import remains",
    );
  }
  ...
  const limiterFrozen = /RETAINED FROZEN/.test(otpLimiter);
  if (otpLimiter.length > 0 && !limiterFrozen) {
    problems.push("otpLimiter.ts lost its RETAINED FROZEN marker — it must not be re-wired");
  }

  const emailOtpReappeared = (files["src/convex/auth/emailOtp.ts"] ?? "").length > 0;
  const emailDeliveryReappeared = (files["src/convex/lib/emailDelivery.ts"] ?? "").length > 0;
  if (emailOtpReappeared) problems.push("retired src/convex/auth/emailOtp.ts reappeared");
  if (emailDeliveryReappeared) problems.push("retired src/convex/lib/emailDelivery.ts reappeared");

  return {
    neverCallsFreebuffHost: neverCalls,
    usesXstarzEmailAbstraction: otpProviderRetired && !emailOtpReappeared && !emailDeliveryReappeared,
    retiredIssuersRefusedInProduction: retiredListed && productionRefusesFederated,
    productionEmailFailsClosed: !emailDeliveryReappeared && limiterFrozen,
    noFreebuffOtpFallback: neverCalls && otpProviderRetired,
    problems,
  };
}
