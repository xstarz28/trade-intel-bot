...
describe("Phase 218 — audited surfaces that were already correct stay correct", () => {
  ...
  it("the auth page does not navigate before the server confirms the session", () => {
    ...
    // Phase 269 removed the email/OTP handler this assertion used to slice.
    // The property under test is unchanged and now guards the remaining
    // interactive handlers (Google + guest): none of them redirects inline —
    // the redirect is driven by isAuthenticated in an effect, never before
    // the server confirms the session.
    const handlers = auth.slice(
      auth.indexOf("const handleGoogleLogin"),
      auth.indexOf("return ("),
    );
    expect(handlers).not.toMatch(/navigate\(/);
    // And the Google handler really exists — the slice must not be vacuous.
    expect(auth).toContain("const handleGoogleLogin");
  });
});
